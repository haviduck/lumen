...
std::unique_ptr<WebviewHost> WebviewHost::Create(
  ...
  // Always create options so we can opt into browser-extension support.
  // `ICoreWebView2EnvironmentOptions6::put_AreBrowserExtensionsEnabled`
  // MUST be called before the environment is constructed; toggling it
  // post-creation has no effect. The QI is a no-op on older runtimes
  // (graceful degrade — extensions just stay off).
  //
  // The type is `wil::com_ptr` (not `Microsoft::WRL::ComPtr`) because
  // we need `.get()` + `.try_query<>()` below; WRL::ComPtr only
  // exposes `.As(&)` for QI which would force two-line interop.
  ...
  if (auto opts6 = opts.try_query<ICoreWebView2EnvironmentOptions6>()) {
