...
void Webview::AddBrowserExtension(const std::string& extension_folder_path,
                                  AddBrowserExtensionCallback callback) {
  if (!IsValid()) {
    callback(false, "WebView is not valid");
    return;
  }

  // The profile is the per-WebView2 storage handle that owns extensions.
  // `ICoreWebView2_13::get_Profile` was added in SDK 1.0.1518.46;
  // `ICoreWebView2Profile7::AddBrowserExtension` in 1.0.2535.41. If
  // either QI fails, the runtime is too old or extensions weren't
  // enabled at env-creation time — both are operator errors, not
  // recoverable from here.
  auto webview13 = webview_.try_query<ICoreWebView2_13>();
  if (!webview13) {
    callback(false,
             "ICoreWebView2_13 unavailable (WebView2 runtime too old)");
    return;
  }
  wil::com_ptr<ICoreWebView2Profile> profile;
  if (FAILED(webview13->get_Profile(profile.put())) || !profile) {
    callback(false, "Failed to obtain WebView2 profile");
    return;
  }
  auto profile7 = profile.try_query<ICoreWebView2Profile7>();
  if (!profile7) {
    callback(false,
             "ICoreWebView2Profile7 unavailable (extensions not supported "
             "by this WebView2 runtime)");
    return;
  }

  HRESULT hr = profile7->AddBrowserExtension(
      util::Utf16FromUtf8(extension_folder_path).c_str(),
      Callback<ICoreWebView2ProfileAddBrowserExtensionCompletedHandler>(
          [callback](HRESULT result,
            ...
            if (FAILED(result) || !extension) {
              callback(false,
                       std::format("AddBrowserExtension failed (HRESULT 0x{:08x})",
                                   static_cast<uint32_t>(result)));
              return S_OK;
            }
            wil::unique_cotaskmem_string id;
            if (SUCCEEDED(extension->get_Id(&id)) && id) {
              callback(true, util::Utf8FromUtf16(id.get()));
            } else {
  ...
  if (FAILED(hr)) {
    callback(false,
             std::format("AddBrowserExtension call failed (HRESULT 0x{:08x})",
