...
class Webview {
 public:
  ...
  // (success, error_message_or_extension_id)
  typedef std::function<void(bool, const std::string&)>
      AddBrowserExtensionCallback;
  ...
  // Loads an unpacked Chromium extension from a local folder. The path
  // must be a directory containing manifest.json (and the rest of the
  // unpacked extension files). Requires `AreBrowserExtensionsEnabled`
  // to have been set on the env options at WebView2 creation, otherwise
  // the underlying call returns ERROR_NOT_SUPPORTED.
  void AddBrowserExtension(const std::string& extension_folder_path,
