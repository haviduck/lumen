...
class WebviewController extends ValueNotifier<WebviewValue> {
  ...
  /// Loads an unpacked Chromium extension from a local folder.
  ///
  /// [extensionFolderPath] must be an absolute path to a directory
  /// containing the extension's `manifest.json` and the rest of its
  /// unpacked files. Returns the extension's id string on success.
  ///
  /// Requires:
  ///  - WebView2 Runtime supporting `ICoreWebView2Profile7` (~ runtime
  ///    121 / SDK 1.0.2535.41 or newer).
  ///  - `AreBrowserExtensionsEnabled` set on the env options at WebView2
  ///    creation time. The vendored `webview_host.cc` opts into this
  ///    unconditionally; nothing for the caller to do.
  ///
  /// Throws a `PlatformException` (kind `method_failed`) when the
  /// underlying `ICoreWebView2Profile7::AddBrowserExtension` call fails.
  /// The most common failure modes are: WebView2 runtime too old, the
  /// folder doesn't contain a valid manifest, or the manifest declares
  /// permissions WebView2 forbids.
  ///
  /// see https://learn.microsoft.com/en-us/microsoft-edge/webview2/reference/win32/icorewebview2profile7
  Future<String?> addBrowserExtension(String extensionFolderPath) async {
    if (_isDisposed) {
      return null;
    }
    assert(value.isInitialized);
    return _methodChannel.invokeMethod<String>(
        'addBrowserExtension', extensionFolderPath);
  }
