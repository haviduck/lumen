...
void WebviewBridge::HandleMethodCall(
  ...
  // addBrowserExtension: string (absolute path to unpacked extension folder)
  // Returns the extension id on success, or an error on failure.
  if (method_name.compare(kMethodAddBrowserExtension) == 0) {
    if (const auto path = std::get_if<std::string>(method_call.arguments())) {
      ...
      webview_->AddBrowserExtension(
          *path,
          [shared_result](bool success, const std::string& id_or_error) {
            if (success) {
              shared_result->Success(
                  flutter::EncodableValue(id_or_error));
            } else {
              shared_result->Error(kMethodFailed, id_or_error);
            }
          });
      return;
    }
